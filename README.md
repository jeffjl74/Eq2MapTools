...
|<nobr>/show_map_style_name&nbsp;1</nobr>       | Turns on the generation of a log file entry for the internal name of a zone each time you zone somewhere. Logging the map style name helps with the [generation of an accurate mapstyles xml element](#copy-button) on the [Zone Rect Tab](#zone-rect-tab).|
